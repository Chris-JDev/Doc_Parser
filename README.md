...
## Deploy: Render + GitHub Pages + Ollama Cloud

### 1) Render (backend)

This repo includes a `Dockerfile` that installs Poppler (required for `pdf2image`) and runs FastAPI.

**Render settings**
- Environment: **Docker**
- Start command: defined in `Dockerfile`
- Add a **Persistent Disk** (recommended) and set:
  - `DATA_DIR=/var/data` (or your mount path)

**Environment variables**
- `OLLAMA_BASE_URL=https://ollama.com/api`
- `OLLAMA_API_KEY=...` (keep this only on the backend)
- `VISION_MODEL=qwen3-vl:235b-cloud`
- `STRUCTURING_MODEL=phi4-json` (or a cloud tag if you use one)
- `FRONTEND_ORIGINS=https://<your_user>.github.io,https://<your_user>.github.io/<your_repo>`

### 2) GitHub Pages (frontend)

A static frontend is included in `docs/`.
...
Open the Pages URL, paste your Render backend URL, upload PDFs, and track jobs.
